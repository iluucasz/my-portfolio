...
const ProjectList = ({ src, label, description }: TProjectList) => {
  return (
    <div className="group relative cursor-pointer items-center justify-center rounded-xl overflow-hidden  transition-shadow hover:shadow-xl border-2 border-transparent hover:border-2 hover:border-red-900 hover:shadow-black/30">
      <div className="h-96 w-72">
        <Image className="h-full w-full object-cover transition-transform duration-500 group-hover:rotate-3 group-hover:scale-125" src={src} alt={label} />
      </div>

      <div className="absolute inset-0 bg-gradient-to-b from-transparent via-transparent to-black group-hover:from-black/70 group-hover:via-black/60 group-hover:to-black/70"></div>

      <div className="absolute inset-0 flex translate-y-[60%] flex-col items-center justify-center px-9 text-center transition-all duration-500 group-hover:translate-y-0">
        <h1 className="flex gap-1 items-center font-dmserif text-3xl font-bold text-red-900"><MdOutlineTag />{label}</h1>
